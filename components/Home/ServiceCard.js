function ServiceCard({ Count, Title, Description, buttonText, background }) {
  return (
    <div className="">
      <div
        className={`${background} bg-secondaryColor bg-opacity-20 p-5 rounded-md my-6 flex flex-col items-center 
      ...
      >
        ...
        {buttonText && (
          <div className="mt-5">
            <button classsName="text-center mt-2 font-text text-sm font-bold text-lightColor bg-secondaryColor py-1 px-4 rounded-lg">
              {buttonText}
            </button>
          </div>
        )}
